...
export function WellbeingChart({ data }: WellbeingChartProps) {
	...
	const points = data.map((point, index) => {
		const x =
			HORIZONTAL_PADDING + (innerWidth * index) / Math.max(data.length - 1, 1);
		const y =
	...
	return (
		<View>
			<Svg width={chartWidth} height={CHART_HEIGHT}>
				<Defs>
					<LinearGradient id="wellbeingArea" x1="0%" y1="0%" x2="0%" y2="100%">
						<Stop offset="0%" stopColor="rgba(197,193,245,0.42)" />
						<Stop offset="100%" stopColor="rgba(197,193,245,0.02)" />
					</LinearGradient>
					<LinearGradient id="wellbeingLine" x1="0%" y1="0%" x2="100%" y2="0%">
