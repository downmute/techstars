...
class ParakeetTokenizer {
	...
	constructor(
		...
		this.blankId =
			blankId ??
			(discoveredBlankId >= 0 ? discoveredBlankId : idToToken.length);
	...
	decode(
		ids: readonly number[],
		options: { skipControlTokens?: boolean } = {},
	) {
		return ids
			.filter((id) => id !== this.blankId)
			.filter(
				(id) => !options.skipControlTokens || !this.controlTokenIds.has(id),
			)
...
class JSMelProcessor {
	...
	process(audio: Float32Array) {
		...
		if (
			!this.rawMelBuffer ||
			this.rawMelBuffer.length < this.nMels * frameCount
		) {
...
function parseConfig(content: string): ParakeetConfig {
	...
	return {
		...
		melBins: Number(
			parsed.melBins ?? parsed.mel_bins ?? DEFAULT_CONFIG.melBins,
		),
		frameShiftSeconds: Number(
		...
		blankId: Number(
			parsed.blankId ?? parsed.blank_id ?? DEFAULT_CONFIG.blankId,
		),
...
class ParakeetOrtRuntime {
	...
	constructor(
		...
		this.encoderAudioInputName = encoderSession.inputNames.includes(
			"audio_signal",
		)
			? "audio_signal"
		...
		this.decoderState1InputName = decoderSession.inputNames.includes(
			"input_states_1",
		)
			? "input_states_1"
			: decoderSession.inputNames[2]!;
		this.decoderState2InputName = decoderSession.inputNames.includes(
			"input_states_2",
		)
			? "input_states_2"
		...
		this.state1OutputName = decoderSession.outputNames.includes(
			"output_states_1",
		)
			? "output_states_1"
		...
		this.state2OutputName = decoderSession.outputNames.includes(
			"output_states_2",
		)
			? "output_states_2"
			: (decoderSession.outputNames.find(
		...
		this.decoderState1Shape = getTensorInputShape(
			decoderSession,
			this.decoderState1InputName,
			[this.config.predLayers, 1, this.config.predHidden],
		);
		this.decoderState2Shape = getTensorInputShape(
			decoderSession,
			this.decoderState2InputName,
			[this.config.predLayers, 1, this.config.predHidden],
		);
	}
	...
	async transcribeDetailed(
		...
		try {
			...
			for (let frameIndex = 0; frameIndex < encodedFrames; frameIndex += 1) {
				...
				while (emittedOnFrame < this.config.maxSymbolsPerStep) {
					...
					if (this.decoderTargetLengthInputName && targetLengthTensor) {
						decoderFeeds[this.decoderTargetLengthInputName] =
							targetLengthTensor;
					}
					...
					const { tensor: logitsTensor } = getTensorFromOutputs(
						decoderOutputs,
						[this.logitsOutputName, "outputs"],
					);
			...
			const text = this.tokenizer
				.decode(tokenIds, { skipControlTokens: true })
				.trim();
			if (text) {
				console.log(
...
export async function initParakeetRuntime(): Promise<boolean> {
	...
	initPromise = (async () => {
		try {
			...
			console.log(
				"[Parakeet] files present, creating tokenizer and ONNX sessions",
			);
