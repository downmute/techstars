...
import { transcribeAudioBufferDetailed, transcribeDetailed } from './stt-engine';
...
function concatenateAudioChunks(chunks: Float32Array[], sampleCount: number): Float32Array {
...
async function ensureNativeCaptureStopped(): Promise<void> {
  if (Platform.OS !== 'ios' || !isNativeAudioCaptureAvailable() || !nativeCaptureStarted) {
    return;
  }
...
async function ensureNativeCaptureStarted(): Promise<void> {
  if (Platform.OS !== 'ios' || !isNativeAudioCaptureAvailable() || !passiveListeningEnabled) {
    return;
  }
...
async function streamAssistantReply(messages: ChatMessage[]): Promise<void> {
  ...
  const maybeStartSpeech = () => {
    ...
    speechPromise = drainSentenceQueue(generation, speechQueue, (amplitude) => {
      ...
      speechPromise = null;
      if (finalized && speechQueue.length === 0 && generation === turnGeneration) {
        setState('idle');
...
function trimLiveAudioIfNeeded() {
  while (liveAudioSampleCount > LIVE_MAX_UTTERANCE_SAMPLES && liveAudioChunks.length > 1) {
...
function pushLivePreRollChunk(chunk: Float32Array) {
  ...
  while (livePreRollSampleCount > LIVE_PRE_ROLL_SAMPLES && livePreRollChunks.length > 1) {
...
function maybeDecodeLiveUtterance(trigger: 'periodic' | 'silence') {
  ...
  if (!isSilenceTrigger) {
    ...
    if (liveAudioSampleCount - liveLastDecodeSampleCount < LIVE_DECODE_INTERVAL_SAMPLES) {
      return;
    }
...
async function handleNativeChunk(
  event: { pcm16Base64: string; frameCount: number }
): Promise<void> {
  if (!passiveListeningEnabled) {
    return;
  }
...
async function runPassiveListeningLoop(): Promise<void> {
  while (passiveListeningEnabled) {
    ...
    try {
      ...
      const likelySilent =
        typeof result.metering === 'number' && result.metering < MIN_SILENCE_METERING;

      if (likelySilent && !pendingTranscript) {
        console.log(`[VoicePipeline] slice=${result.sliceId} skipped as likely silence`);
        setState('idle');
        continue;
      }
      ...
      if (shouldFlush) {
        console.log(`[VoicePipeline] flushing pending transcript="${pendingTranscript.slice(0, 160)}"`);
