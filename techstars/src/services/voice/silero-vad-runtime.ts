...
class SileroVadRuntime {
  ...
  constructor(readonly session: OrtSession) {
    const metas = normalizeSessionInputMetas(
      (session.inputMetadata as Parameters<typeof normalizeSessionInputMetas>[0]) ??
        []
    );

    const inputMeta =
      metas.find((meta) => meta.name === 'input') ??
      metas.find((meta) => meta.elemType === 'float32' && !meta.name.includes('state')) ??
      metas[0];
    const stateMeta =
    ...
    this.stateOutputName = session.outputNames.includes('stateN')
      ? 'stateN'
      : session.outputNames.find((name) => name.toLowerCase().includes('state')) ??
...
export class SileroVadStream {
  ...
  private async processFrame(frame: Float32Array): Promise<SileroVadFrameResult> {
    ...
    const inputTensor = new OrtTensor(
      'float32',
      inputData,
      [1, inputData.length]
    );
    ...
    try {
      ...
      probabilityTensor =
        outputs[this.runtime.outputName] ?? outputs[this.runtime.session.outputNames[0]!]!;
      nextState =
      ...
      if (!this.triggered && this.speechFrameStreak >= SPEECH_START_FRAMES) {
        ...
        this.silenceFrameStreak = 0;
        speechStarted = true;
      } else if (this.triggered && this.silenceFrameStreak >= SPEECH_END_FRAMES) {
        this.triggered = false;
        this.speechFrameStreak = 0;
        this.silenceFrameStreak = 0;
        speechEnded = true;
