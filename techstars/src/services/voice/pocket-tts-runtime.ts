...
class ONNXBackboneAdapter {
	...
	setAllInputMetas(metas: OnnxInputMeta[]) {
		...
		for (const meta of metas) {
			...
			if (meta.name === "sequence") {
				...
				meta.name === "text_embeddings" ||
				meta.name.includes("text")
			) {
	...
	private emptyEmbFeeds(exceptName?: string) {
		...
		if (this.textEmbName !== exceptName) {
			feeds[this.textEmbName] = new OrtTensor("float32", new Float32Array(0), [
				1,
				0,
				this.textEmbDim,
			]);
		...
		if (this.voiceEmbName && this.voiceEmbName !== exceptName) {
			feeds[this.voiceEmbName] = new OrtTensor("float32", new Float32Array(0), [
				1,
				0,
				this.voiceEmbDim,
			]);
	...
	async conditionVoice(voiceData: Float32Array, shape: readonly number[]) {
		...
		await this.run({
			sequence: new OrtTensor("float32", new Float32Array(0), [
				1,
				0,
				this.seqDim,
			]),
	...
	async stepAR(seq: Float32Array) {
		...
		const eosTensor =
			outputs.eos_logit ?? outputs[this.session.outputNames[1]!]!;
...
function parseGraphProtoAllInputs(
	bytes: Uint8Array,
	start: number,
	end: number,
) {
...
function tokenize(
	...
	for (let i = 0; i < length; i += 1) {
		...
		for (
			let subLength = 1;
			subLength <= Math.min(maxPieceLen, length - i);
			subLength += 1
		) {
...
export async function initPocketTTSRuntime(): Promise<boolean> {
	...
	const baseInitPromise = (async () => {
		...
		const textConditionerPath = toNativeFsPath(
			getModelPath("text_conditioner.onnx"),
		);
...
export async function synthesizeWithPocketTTS(
	...
	await runtime.backbone.conditionVoice(
		runtime.voice.data,
		runtime.voice.shape,
	);
